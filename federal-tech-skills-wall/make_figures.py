...
plt.rcParams.update({"font.size": 11, "axes.grid": True, "axes.axisbelow": True,
                     "grid.alpha": .25, "figure.dpi": 150})
FED_C = "#c0392b"; PRIV_C = "#2471a3"
OUT = "."  # writes PNGs to cwd
os.makedirs(OUT, exist_ok=True)

# ---- FEDERAL = Skillenai index (federal_now.json); PRIVATE = Skillenai index (private_*.json) ----
now = json.load(open("federal_now.json"))
def series(h):
    m = re.search(r"\b(2210|1550|1560|0854|0855|1515|1529|1530)\b", h.get("extractedText") or "")
    return m.group(1) if m else None
for h in now:
    h["_s"] = series(h); h["_tl"] = (h.get("title") or "").lower()
def ftxt(mask): return [h.get("extractedText") or "" for h in now if mask(h)]
FDS = ftxt(lambda h: h["_s"] == "1560" or "data scien" in h["_tl"])
FSWE = ftxt(lambda h: h["_s"] in ("2210", "1550") and re.search(r"software|developer|programmer|application|appsw|full stack|web ", h["_tl"]))
FSEC = ftxt(lambda h: re.search(r"cyber|security|infosec|information security|information assurance", h["_tl"]))
priv = {k: [(h.get("extractedText") or "") for h in json.load(open(f"private_{k}.json"))] for k in ["ds", "swe", "sec"]}

def rate(texts, pat):
    rx = re.compile(pat, re.I); n = len(texts)
    return 100*sum(1 for t in texts if rx.search(t))/n if n else 0

# ---------- FIG 1: the missing roles ----------
fig, ax = plt.subplots(figsize=(9, 5.2))
roles = ["Data\nScientist", "Data\nAnalyst", "Data\nEngineer", "ML\nEngineer", "AI\nEngineer"]
fed_ct = [129, 2, 17, 3, 5]              # our federal index, title match (~1,345 tech postings)
priv_ct = [2181, 1038, 1758, 2555, 848]  # private index counts
x = np.arange(len(roles)); w = .4
ax.bar(x-w/2, fed_ct, w, color=FED_C, label="Federal (Skillenai index, ~1,345 tech postings)")
...
ax.set_title("The modern data org barely exists in federal hiring\nSkillenai job-index counts by title, federal vs private", fontweight="bold")
...
fedv = [rate(FDS, p[1]) for p in DS_PROBES]; prv = [rate(priv["ds"], p[1]) for p in DS_PROBES]
...
ax.xaxis.set_major_formatter(PercentFormatter()); ax.set_xlabel("% of postings mentioning")
...
ax.axhline(5.5, color="k", lw=.8, ls="--", alpha=.5); ax.legend(loc="lower right")
...
ax.xaxis.set_major_formatter(PercentFormatter()); ax.set_xlabel("Share of tech postings")
ax.set_title("Role families invert between sectors", fontweight="bold"); ax.legend()
...
ax = axes[1]
sers = ["2210 IT Mgmt", "1560 Data Science", "1515 Ops Research", "1550 Comp Science",
        "0854 Comp Eng", "1529/1530 Stats", "0855 Electronics"]
vals = [81.0, 9.0, 1.3, 1.6, 1.0, 0.9, 0.4]
ax.barh(np.arange(len(sers))[::-1], vals, color=[FED_C] + ["#e59866"]*6)
ax.set_yticks(np.arange(len(sers))[::-1]); ax.set_yticklabels(sers)
ax.xaxis.set_major_formatter(PercentFormatter()); ax.set_xlabel("Share of federal tech postings")
ax.set_title("One OPM series (2210) is 81% of federal tech", fontweight="bold")
...
SWE_P = [("RMF / NIST / FISMA", r"\brmf\b|\bnist\b|fisma|fedramp|risk management framework"),
         ...
         ("React / Node", r"\breact\b|node\.?js"), ("CI/CD / microservices", r"ci/cd|microservice"),
...
SEC_P = [("Security clearance", r"security clearance|clearance"), ("RMF / A&A / ATO", r"\brmf\b|authorization to operate|assessment and authorization"),
         ("NIST", r"\bnist\b"), ("Python", r"\bpython\b"), ("AWS / GCP", r"\baws\b|\bgcp\b|google cloud"),
...
for ax, probes, ftx, ptx, title in [
    (axes[0], SWE_P, FSWE, priv["swe"], "Software Engineer\ncompliance overlay (fed) vs cloud-native (priv)"),
    (axes[1], SEC_P, FSEC, priv["sec"], "Cybersecurity\nclearance+governance (fed) vs hands-on (priv)")]:
...
ax = axes[0]
fams2 = ["Security/Cyber", "Program/Mgmt", "IT/Ops/Infra", "Data/ML/AI", "Software/Dev", "ALL federal tech"]
clr = [75, 61, 55, 30, 8, 58]
ax.barh(np.arange(len(fams2))[::-1], clr, color=FED_C)
ax.axvline(8, color=PRIV_C, ls="--", lw=1.5)
...
for i, v in zip(np.arange(len(fams2))[::-1], clr):
    ax.text(v+1, i, f"{v:.0f}%", va="center", fontsize=9, fontweight="bold")
ax = axes[1]
trend = json.load(open("remote_trend.json"))
...
print("done. federal subsets: DS=%d SWE=%d SEC=%d" % (len(FDS), len(FSWE), len(FSEC)))
